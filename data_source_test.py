...
def print_summary_report(all_results):
    ...
    for data_source, results in all_results.items():
        ...
        if successful_tests > 0:
            avg_response_time = sum(r['response_time_ms'] for r in results.values() if r['status'] == '成功') / successful_tests
            avg_data_quality = sum(r['data_quality']['score'] for r in results.values() if r['status'] == '成功') / successful_tests
